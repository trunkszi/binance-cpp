        ...
        struct exchange_info_t {
            ...
            struct symbol_t {
                ...
                struct filter_t {
                    ...
                    boost::variant<
                            price_t, percent_price_t, percent_price_by_side_t, lot_size_t, market_lot_size_t, min_notional_t, iceberg_parts_t, max_num_orders_t, max_num_algo_orders_t, max_position_t, trailing_delta_t
                    > filter;
                ...
                const filter_t::percent_price_t &
                get_filter_percent_price() const { return get_filter<filter_t::percent_price_t>(); }

                const filter_t::percent_price_by_side_t &
                get_filter_percent_price_by_side() const { return get_filter<filter_t::percent_price_by_side_t>(); }

                const filter_t::lot_size_t &get_filter_lot_size() const { return get_filter<filter_t::lot_size_t>(); }

                const filter_t::market_lot_size_t &
                get_filter_market_lot_size() const { return get_filter<filter_t::market_lot_size_t>(); }

                const filter_t::min_notional_t &
                get_filter_min_notional() const { return get_filter<filter_t::min_notional_t>(); }

                const filter_t::iceberg_parts_t &
                get_filter_iceberg_parts() const { return get_filter<filter_t::iceberg_parts_t>(); }

                const filter_t::max_num_orders_t &
                get_filter_max_num_orders() const { return get_filter<filter_t::max_num_orders_t>(); }

                const filter_t::max_num_algo_orders_t &
                get_filter_max_num_algo_orders() const { return get_filter<filter_t::max_num_algo_orders_t>(); }

                const filter_t::max_position_t &
                get_filter_max_position() const { return get_filter<filter_t::max_position_t>(); }

                const filter_t::trailing_delta_t &
                get_filter_trailing_delta() const { return get_filter<filter_t::trailing_delta_t>(); }
...
// https://github.com/binance-exchange/binance-official-api-docs/blob/master/rest-api.md#test-new-order-trade
        ...
        struct new_order_resp_type : boost::variant<new_order_info_ack_t, new_order_info_result_t, new_order_info_full_t, new_test_order_info_t > {
            // ctor inheritance
            using boost::variant<
                    new_order_info_ack_t, new_order_info_result_t, new_order_info_full_t, new_test_order_info_t
            >::variant;
